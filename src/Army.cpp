...
void Army::Hit(float damage) {
    HP -= damage;
    if (damage == 1.5 && this->speed > 10) this->speed-=10;
    if (HP <= 0) {
        OnExplode();
    ...
    if (region == 5) {
        ...
        if (!Target) {
            ...
            for (auto& it : scene->DefenseGroup->GetObjects()) {
                ...
                if (dis < maxDis&& dynamic_cast<Defense*>(it)->id!=2) {
                    maxDis = dis;
        ...
        if (Target) {
            if (id != 3)Rotation = UpdateRotation(deltaTime, Target->Position);
            // Shoot reload.
            reload -= deltaTime;
            if (reload <= 0) {
                // shoot.
        ...
        if (!movingToWall) {
            ...
            if (id != 3)Rotation = UpdateRotation(deltaTime, wallPos);
            // Shoot reload.
            reload = coolDown;
            // position
            float wx = wallPos.x;
