...
void PlayScene::Initialize() {
    ...
    AddNewObject(UItime = new Engine::Label("TIME:120", "pirulen.ttf", 20.5, 1300, 150, 60, 60, 60, 255, 0.5, 0.5));
	ReadMap();
	ConstructUI();
	...
	// Preload Lose Scene
	deathBGMInstance = Engine::Resources::GetInstance().GetSampleInstance("harryLose.ogg");
	Engine::Resources::GetInstance().GetBitmap("lose/voldemort.jpg");

	// Start BGM.
    if (!mute)
        bgmInstance = AudioHelper::PlaySample("hogwarts2.ogg", true, AudioHelper::BGMVolume);
    else
        bgmInstance = AudioHelper::PlaySample("hogwarts2.ogg", true, 0.0);
}
...
void PlayScene::Update(float deltaTime) {
    ...
    timer -= deltaTime;
    int inttime = timer / 1;
    UItime->Text.erase(5, UItime->Text.length()-5);
    UItime->Text.insert(5, std::to_string(inttime));
    // Win
    if (DefenseGroup->GetObjects().empty()) {
        Engine::GameEngine::GetInstance().ChangeScene("win");
    }
    if (DefenseGroup->GetObjects().size() <= invisiblenum) {
        int trapnum = 0;
        for (auto& it : DefenseGroup->GetObjects()) {
            if (dynamic_cast<Defense*>(it)->id == 2)trapnum++;
            else {
                trapnum = 0;
                break;
            }
        }
        if (trapnum == DefenseGroup->GetObjects().size()) {
            allDie = true;
        }
    }
		...
		if (!CheckOccupied(x, y)|| preview->id>=3) {
            ...
            if (remainId != -1) {
                if (remainId == 0)
                    ...
                    preview = new BombArmy(1, 0);
                else if (remainId == 2)
                    preview = new RonArmy(2, 0);
                else if (remainId == 3) 
                    preview = new IceSpell(3, 0);
                else if (remainId == 4) 
                    preview = new HeroArmy(4, 0);
                preview->Position = Engine::GameEngine::GetInstance().GetMousePosition();
...
void PlayScene::ReadMap() {
	...
	while (fin >> c) {
        ...
        case '4': mapData.push_back(TILE_INVISIBLE); invisiblenum++; break;
        case '5': mapData.push_back(TILE_DRACO); break;
		case '\n':
		case '\r':
		...
		for (int j = 0; j < MapWidth; j++) {
            ...
            switch (md) {
                ...
                case TILE_CANNON:
                    ...
                    break;
                case TILE_CANNON2:
                    DefenseGroup->AddNewObject(new CannonDefense2(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
                    break;
                case TILE_INVISIBLE:
                    DefenseGroup->AddNewObject(new InvisibleDefense(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
                    break;
                case TILE_DRACO:
                    DefenseGroup->AddNewObject(new DracoDefense(j * BlockSize + BlockSize / 2, i * BlockSize + BlockSize / 2));
                    break;
                case TILE_FLOOR:
                    if (j <= MapWidth-2 && j >= 2) {
...
bool PlayScene::CheckOccupied(int x, int y) {
    ...
    if (tt == TILE_WALL || tt == TILE_CANNON || tt == TILE_CANNON2||tt== TILE_DRACO) return true;
