...
void ArmySelectScene::Initialize() {
    ...
    AddNewObject(UISpaceUsage = new Engine::Label("Space: /30", "pirulen.ttf", 30, 1395, 150, 0, 0, 0, 255, 0.5, 0.5));
    ...
    UISpaceUsage->Text.insert(7, std::to_string(usedSpace));
    fromSetting = false;
    //armyAmount[0] = 4;
    
    // set ArmyImage
    ArmyImage[0] = "play/harry.png";
    // TODO 2 (2/8): Create the bomb image. You can find image in the play/ folder.
    ArmyImage[1] = "play/hermione.png";
    ArmyImage[2] = "play/ron.png";
    ArmyImage[3] = "play/dumbledore.png";
    ...
    // play bgm
    if (!mute)
        bgmInstance = AudioHelper::PlaySample("harry.ogg", true, AudioHelper::BGMVolume);
    else
        bgmInstance = AudioHelper::PlaySample("harry.ogg", true, 0.0);
}
...
void ArmySelectScene::PlayOnClick(ButtonType type, int id, int spaceCost) {
    if (type == BUTTON_ENTER) {
        ...
        armyAmount[3] = 2;
        armyAmount[4] = 1;
        UISpaceUsage->Text.erase(7, 2).insert(7, std::to_string(usedSpace));
    }
        ...
        if (usedSpace == totalSpace)return;
        usedSpace++;
        armyAmount[id]++;
        UISpaceUsage->Text.erase(7, UISpaceUsage->Text.length()-10).insert(7, std::to_string(usedSpace));
...
void ArmySelectScene::AddNewArmy(int id, std::string imageName, int spaceCost) {
    ...
    AddNewControlObject(btn);
    AddNewObject(new Engine::Label("+", "pirulen.ttf", 30, (halfW / 4 + 37 + offsetW), (oneThirdH + 50 + offsetH), 0, 0, 0, 255, 0.5, 0.5));
    ...
    AddNewControlObject(btn);
    AddNewObject(new Engine::Label("-", "pirulen.ttf", 30, (halfW / 4 + 137 + offsetW), (oneThirdH + 50 + offsetH), 0, 0, 0, 255, 0.5, 0.5));
}
